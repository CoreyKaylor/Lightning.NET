...
make clean
docker run --mount type=bind,source=$(pwd),target=/lmdb --rm --platform linux/arm/7 arm-gcc
mv ./liblmdb.so ../../../../src/LightningDB/runtimes/linux-arm/native/liblmdb.so
make clean
docker run --mount type=bind,source=$(pwd),target=/lmdb --rm --platform linux/arm64 arm64-gcc
mv ./liblmdb.so ../../../../src/LightningDB/runtimes/linux-arm64/native/liblmdb.so
make clean
docker run --mount type=bind,source=$(pwd),target=/lmdb --rm --platform linux/amd64 amd64-gcc
